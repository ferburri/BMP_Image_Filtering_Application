/* 	
	Parallel Programm for a BMP file processing "image-par.cpp"
...
// Function to transfor either the data array into the data matrix or viceversa.
...
void gaussOperation (Pixel **imInitial, Pixel **imFinal, int height, int width) {
	// Parallelization of the code regarding the gaussian operation.
	// The parallelization will include different for's, so they will need to collapse both of them (i, j)
	// to derive each pixel to a different thread.
	#pragma omp parallel 
	{
